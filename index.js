...
function cesitEkle(originalTatlar) {
  const kopyaTatlar = [...originalTatlar];
...
function sonCesitiKaldir(originalTatlar) {
  const kopyaTatlar = [...originalTatlar];
  kopyaTatlar.pop();
  return kopyaTatlar;
}
...
const tatlar = ["Muz", "Vişne", "Ceviz", "Kestane", "Kiraz", "Çikolata"];
...
*/
const yeni = "Vanilya";
function ismeGoreFiltrele(tatlar, yeni) {
  return tatlar.filter((tatlar) => tatlar.includes(yeni));
}
...
function ortalamaKelimeSayisi(originalTatlar) {
  const kelimeler = originalTatlar.split(" ");
  const toplamKelime = kelimeler.length;
  const ortalama = toplamKelime / kelimeler.length;
  return ortalama;
}
